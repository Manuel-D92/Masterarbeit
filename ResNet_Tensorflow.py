...
class config_Test_Tensorflow():
    ...
    sequence_training_cut_y = [-200,14000]
    sequence_training_cut_z = [-600,600]

    #### Testdata into Trainingssequence
    sequence_test = True
    sequence_test_cut_x = [-200,1700]
    sequence_test_cut_y = [-200,14000]
    sequence_test_cut_z = [-600,600]
...
class ResNet_Tensorflow:
    ...
    def __init__(self, creat_dataset_time_steps, time_steps_geo_folge, create_dataset_overlap, create_dataset_offset,
                 sequence_training, sequence_training_cut_x, sequence_training_cut_y, sequence_training_cut_z,
                 sequence_test, sequence_test_cut_x, sequence_test_cut_y, sequence_test_cut_z, path, path_test_sequenz,
                 filename_training, filename_test_sequenz, save_dir,
                 all_or_range_selection_Data, training_Data_from, training_Data_to, time, X_Signal_number, Input_from,
                 Input_to, Output_from, Output_to, trainings_epochs, batch_size, verbose, metrics, learning_rate,
                 optimizer_name,
                 optimizer, split_test_size, split_KFold_set, split_random_state, split_shuffle, loss_funktions, scaler,
                 threshold_antenna, threshold_value, pickle, png, kernel_init, bias_init):
        # self.a = outside.t
        # Time Steps (create_dataset)
            ...
            for c in range(0, len(Y_test)):
                ...
                # elif (flag == 1):
                    ...
                    if (self.sequence_training_cut_y[0] > Y_test[c, 1]):
                        ...
                        over_under_y = Y_test[c, 1]
                    if (self.sequence_training_cut_z[0] > Y_test[c, 2]):
                        # under
                        over_under_z = self.sequence_training_cut_z[0] - 10
                    elif (Y_test[c, 2] > self.sequence_training_cut_z[1]):
                        # over
                        over_under_z = self.sequence_training_cut_z[1] + 10
                    else:
                        over_under_z = Y_test[c, 2]
                    X_test_cut_tm.append(X_test[c, :])
    ...
    def shape_the_Signal(self, X_Signal_number_train, X_train_time_series_normalize, Y_train_time_series_normalize, i):
        ...
        Y_train_time_series = []
        first_data = 0
        for j in range(int(X_Signal_number_train[0]),
                       int(X_Signal_number_train[-1] + 1)):  # int(max(X_Signal_number_train))
            of = 0 + count
            ...
            if (self.creat_dataset_time_steps * (2 ** i) < np.count_nonzero(X_Signal_number_train[:] == j)):
                X_train_time_series_tm, Y_train_time_series_tm = ResNet_Tensorflow.create_dataset(
                    X_train_time_series_normalize[of:to][:],
        ...
        if (self.split_test_size * self.split_KFold_set <= 1):
            if (i == 1):
                if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                    ...
                    X_train, Y_train, X_Signal_number_train = ResNet_Tensorflow.cut_Signal_into_sequencen_train(self, X_train,
                                                                                                         Y_train,
                                                                                                         X_Signal_number_train)
                    X_test, Y_test, X_Signal_number_test = ResNet_Tensorflow.cut_Signal_into_sequencen_test(self, X_test,
                                                                                                     Y_test,
                                                                                                     X_Signal_number_test)
            ...
            X_train_time_series, Y_train_time_series = ResNet_Tensorflow.shape_the_Signal(self=self,
                                                                                   ...
                                                                                   i=(i - 1))
            X_test_time_series, Y_test_time_series = ResNet_Tensorflow.shape_the_Signal(self=self,
        ...
        if (i == 1):
            ...
            if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                ...
                X_train, Y_train, X_Signal_number_train = ResNet_Tensorflow.cut_Signal_into_sequencen_train(self, X_train,
                                                                                                     Y_train,
                                                                                                     X_Signal_number_train)
                X_test, Y_test, X_Signal_number_test = ResNet_Tensorflow.cut_Signal_into_sequencen_test(self, X_test, Y_test,
                                                                                                 X_Signal_number_test)
    ...
    def create_training_data_sequence_end_single_signal(self, X_1, Y_1, X_Signal_number, i):
        if (i == 1):
            ...
            X_train, Y_train, X_Signal_number_train = ResNet_Tensorflow.cut_Signal_into_sequencen_train(self, X_train, Y_train,
                                                                                                 X_Signal_number_train)
            X_test, Y_test, X_Signal_number_test = ResNet_Tensorflow.cut_Signal_into_sequencen_test(self, X_test, Y_test,
                                                                                             X_Signal_number_test)
    ...
    def training_NN(self):
        ...
        if (self.all_or_range_selection_Data == True):
            ...
            X_Signal_number = (
                time_xyz_antennen_Signal_Komplex_all_Files[:, self.X_Signal_number:(self.X_Signal_number + 1)].astype(
                    int))
            ...
            X_Signal_number = time_xyz_antennen_Signal_Komplex_all_Files[self.training_Data_from:self.training_Data_to,
                              self.X_Signal_number:(self.X_Signal_number + 1)]
        ...
        for i in range(1, (self.time_steps_geo_folge + 1)):
            ...
            X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series, normalizer_x, normalizer_y = ResNet_Tensorflow.create_training_data_random_KFold(
            ...
            for loss in range(0,len(self.loss_funktions)):
                ...
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_" + str(self.optimizer) + ".png")
                plt.close()
                plt.figure(72)
                acc_history = history.history["mse"][:]
                acc_val_history = history.history["val_mse"][:]
                plt.title("Adam")
                ...
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_MSE" + ".png")
                plt.close()
                ...
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                print_vali.append("Validation Loss")
                plt.legend(print_vali)
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_" + str(self.optimizer) +"_val.png")
                ...
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                print_train.append("Training Loss")
                plt.legend(print_train)
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_" + str(self.optimizer) +"_train.png")
...
if __name__ == '__main__':
    ...
    tensor = ResNet_Tensorflow(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
