...
class config_Test_Tensorflow():
    ...
    training_Data_to = 128  # Only "all_or_range_selection_Data = False"
    time = 1  # 1= Timesteps
    ...
    learning_rate = 0.0007 # 2e-6=0.000 002
    optimizer_name = "Adam"
...
#
    ...
    loss_funktions = ['sparse_categorical_crossentropy'] # ,'huber_loss', sparse_categorical_crossentropy
    # 'logcosh', 'MAPE',
...
class Tensorflow_GoogLeNet:
                ...
                if (self.sequence_training_cut_x[0] < Y_train[c, 0] < self.sequence_training_cut_x[1]
                    ...
                    if ((c < ze) == False or (ze == 0) == True):
                        ...
                        if(zb>=self.creat_dataset_time_steps*2+10  and false_more_peaks==False): # whether Sequence is big enough
                            if (count_out > self.creat_dataset_time_steps):
                                for z_out_count in range(0, count_out - self.creat_dataset_time_steps - 1):
                            ...
                            if(c>=self.creat_dataset_time_steps):
                                in_z_begin = c - self.creat_dataset_time_steps - begin_offset
                                out_z_begin = ze - self.creat_dataset_time_steps - begin_offset
                                class_info_z_begin = c + self.creat_dataset_time_steps - begin_offset
    ...
    def shape_the_Signal(self,X_Signal_number_train,X_train_time_series_normalize,Y_train_time_series_normalize,Class_train,i):
        ...
        for j in range(int(X_Signal_number_train[0]),int(X_Signal_number_train[-1] + 1)): #int(max(X_Signal_number_train))
            ...
            if(self.creat_dataset_time_steps*(2**i)<np.count_nonzero(X_Signal_number_train[:] == j)):
                X_train_time_series_tm, Y_train_time_series_tm,Class_train_tm = Tensorflow_GoogLeNet.create_dataset(X_train_time_series_normalize[of:to][:],
        ...
        if(self.split_test_size*self.split_KFold_set<=1):
            if(i==1):
                if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                    ...
                    X_train, Y_train, X_Signal_number_train = Tensorflow_GoogLeNet.cut_Signal_into_sequencen_train(self, X_train,Y_train,X_Signal_number_train)
                    X_test, Y_test, X_Signal_number_test = Tensorflow_GoogLeNet.cut_Signal_into_sequencen_test(self, X_test,Y_test,X_Signal_number_test)
            ...
            X_train_time_series, Y_train_time_series = Tensorflow_GoogLeNet.shape_the_Signal(self=self,
                                                                                   ...
                                                                                   i=(i - 1))
            X_test_time_series, Y_test_time_series = Tensorflow_GoogLeNet.shape_the_Signal(self=self,
    ...
    def create_training_test_data(self,X_1,Y_1,X_Signal_number,Class,i):
        ...
        if(i==1):
            ...
            if(min_max_interval_split_data*2<len(X_1[0:count_Signal_end, :])):
                ...
                # KFold random
                X_train,X_test,Y_train,Y_test,X_Signal_number_train,X_Signal_number_test,Class_train,Class_test = Tensorflow_GoogLeNet.create_training_test_data_random_KFold(self,X_1,Y_1,X_Signal_number,Class,count_Signal_beginn,count_Signal_end,min_max_interval_split_data)
                ...
                X_train, Y_train, X_Signal_number_train, Class_train = Tensorflow_GoogLeNet.cut_Signal_into_sequencen_train_and_val(self,X_train,Y_train,X_Signal_number_train,Class_train,False)
                X_test, Y_test, X_Signal_number_test, Class_test = Tensorflow_GoogLeNet.cut_Signal_into_sequencen_train_and_val(self, X_test, Y_test,X_Signal_number_test,Class_test,True)
        ...
        X_train_time_series, Y_train_time_series, Class_train = Tensorflow_GoogLeNet.shape_the_Signal(self=self,
                                                                               X_Signal_number_train=X_Signal_number_train,
                                                                               X_train_time_series_normalize=X_train_time_series_normalize,
                                                                               Y_train_time_series_normalize=Y_train_time_series_normalize,
                                                                               Class_train=Class_train,
                                                                               i=(i - 1))
        X_test_time_series, Y_test_time_series, Class_test = Tensorflow_GoogLeNet.shape_the_Signal(self=self,
                                                                             X_Signal_number_train=X_Signal_number_test,
                                                                             X_train_time_series_normalize=X_test_time_series_normalize,
                                                                             Y_train_time_series_normalize=Y_test_time_series_normalize,
    ...
    def create_googLeNet(self,X_train_time_series,Y_train_time_series):
        input_layer = tf.keras.layers.Input(shape=(X_train_time_series.shape[1], X_train_time_series.shape[2]))
        x = tf.keras.layers.Conv1D(64, 11, strides=2, activation='relu', padding="same",
                                   kernel_initializer=self.kernel_init, bias_initializer=self.bias_init)(input_layer)
        x = tf.keras.layers.MaxPooling1D(3, padding='same', strides=2)(x)
        x = tf.keras.layers.Conv1D(64, 7, strides=1, activation='relu', padding="same")(x)
        x = tf.keras.layers.Conv1D(64, 3, strides=1, activation='relu', padding="same")(x)
        x = tf.keras.layers.MaxPooling1D(3, padding='same', strides=2)(x)
        x = Tensorflow_GoogLeNet.inception_module(self=self, x=x,
                                                  filters_1x1=64,
        ...
        x = tf.keras.layers.MaxPooling1D((3), padding='same', strides=(2), name='max_pool_3_3x3/2')(x)
        ...
        x1 = tf.keras.layers.MaxPooling1D((5), padding='same', strides=3)(x)
        x1 = tf.keras.layers.Conv1D(128, (1), padding='same', activation='relu')(x1)
        x1 = tf.keras.layers.Flatten()(x1)
        x1 = tf.keras.layers.Dense(1024, activation='relu')(x1)
        x1 = tf.keras.layers.Dropout(0.5)(x1)
        x1 = tf.keras.layers.Dense(9, activation="softmax",name='auxilliary_output_1')(x1)
        ...
        x2 = tf.keras.layers.MaxPooling1D((5), padding='same', strides=3)(x)
        ...
        x2 = tf.keras.layers.Dense(9,activation="softmax", name='auxilliary_output_2')(x2)
        ...
        x = tf.keras.layers.Dense(9, activation="softmax", name='output')(x)
    ...
    def training_NN(self):
        ...
        for i in range(1, (self.time_steps_geo_folge + 1)):
            ...
            if(self.save==False):
                X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series, normalizer_x, normalizer_y, Class_train, Class_test = Tensorflow_GoogLeNet.create_training_test_data(
                ...
                np.save(self.path+"\\Trainingsset_x_05",Trainings_set_x)
                np.save(self.path + "\\Trainingsset_y_05",Trainings_set_y)
                np.save(self.path + "\\Testset_x_05", Test_set_x)
                np.save(self.path + "\\Testset_y_05", Test_set_y)

            X_train_time_series = np.load(self.path+"\\Trainingsset_x_04.npy")
            Training_set_y = np.load(self.path + "\\Trainingsset_y_04.npy")
            X_test_time_series = np.load(self.path + "\\Testset_x_04.npy")
            Test_set_y = np.load(self.path + "\\Testset_y_04.npy")
            ...
            model = Tensorflow_GoogLeNet.create_googLeNet(self, X_train_time_series, Class_train)
            model.summary()

            time_stamp_from_NN =Tensorflow_GoogLeNet.get_actuall_time_stamp()
            doku = []
            loss = 0
            ...
            for opti in range(0,1):
            #for loss in range(0,len(self.loss_funktions)):
                print('\n\n' + self.loss_funktions[loss])
                model.compile(loss=[self.loss_funktions[loss], self.loss_funktions[loss], self.loss_funktions[loss]],optimizer=self.optimizer[0], metrics=self.metrics)
                start_time_count = time_1.perf_counter()
                history = model.fit(X_train_time_series, [Class_train, Class_train,Class_train],
                                epochs=self.trainings_epochs, shuffle=True, verbose=self.verbose,
                                batch_size=self.batch_size,
                                validation_data=(X_test_time_series, [Class_test, Class_test, Class_test]))  # x=generator
                ...
                Tensorflow_GoogLeNet.plot_2D_Classification(self, model, X_train_time_series, Y_train_time_series, Class_train, 0,
                                                  "Training", path_NN_tm_loss)

                Tensorflow_GoogLeNet.plot_2D_Classification(self, model, X_test_time_series, Y_test_time_series, Class_test, 1,
                                                  "Test", path_NN_tm_loss)


                #doku_training =Tensorflow_GoogLeNet.plot_3D_Classification(self, model, X_train_time_series, Y_train_time_series, Class_train, 0,
                #                                  "Training", path_NN_tm_loss)
                #doku_test = Tensorflow_GoogLeNet.plot_3D_Classification(self, model, X_test_time_series, Y_test_time_series, Class_test, 1,
                #                                  "Test", path_NN_tm_loss)
                ...
                plt.close()
                plt.figure(72)
                acc_history = history.history["output_sparse_categorical_accuracy"][:]
                acc_val_history = history.history["val_output_sparse_categorical_accuracy"][:]
                pl.title(str(opti_name[0]))
                ...
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_Accuracy_" + str(opti_name[0]) + ".png")
                plt.close()
                ...
                plt.xlabel("Epochs")
                plt.ylabel("Accuracy")
                print_vali_acc.append("Validation Accuracy " + str(opti_name[0]))
                plt.legend(print_vali_acc)
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_Accuracy_" + ".png")
                plt.close()
                ...
                plt.xlabel("Epochs")
                plt.ylabel("Accuracy")
                print_train_acc.append("Training Accuracy " + str(opti_name[0]))
                plt.legend(print_train_acc)
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_Accuracy_" + ".png")
                plt.close()
                ...
                plt.xlabel("Epochs")
                plt.ylabel("Loss")
                print_vali.append("Validation Loss " + str(opti_name[0]))
...
if __name__ == '__main__':
    ...
    tensor = Tensorflow_GoogLeNet(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
