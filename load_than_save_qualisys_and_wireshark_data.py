...
class load_than_save_qualisys_and_wireshark_data():
    ...
    def load_than_save_qualisys_and_wireshark_data_compare():
        ...
        Signal_1, Kopf, first_timestamp_wireshark_qualisys = quali.qualisys.load_convert_qulisys_Data(
            ...
            delet_Null=0);
        print("first"+first_timestamp_wireshark_qualisys)
        first_timestamp_wireshark_qualisys = first_timestamp_wireshark_qualisys-0.0359
        print("second" + first_timestamp_wireshark_qualisys)
    ...
    def load_than_save_qualisys_and_wireshark_data_to_learning(self,path, File, which_klass, number_Signal):
        ...
        Signal_1, Kopf, first_timestamp_wireshark_qualisys = quali.qualisys.load_convert_qulisys_Data(
            ...
            delet_Null=0);
        #print("first" , str(first_timestamp_wireshark_qualisys))
        #first_timestamp_wireshark_qualisys = first_timestamp_wireshark_qualisys + 0.1
        #print("second" , str(first_timestamp_wireshark_qualisys))
        timestamp_qualisys = Signal_1[:, 1]
        print("difference between wireshark and qualisys timestamp: ",(timestamp_wireshark[0, 1] - first_timestamp_wireshark_qualisys))
        print("danach: ",(timestamp_wireshark[0, 1] - first_timestamp_wireshark_qualisys) -0.0284)
        timestamp_wireshark[:, 0] = timestamp_wireshark[:, 0] + (timestamp_wireshark[0, 1] - first_timestamp_wireshark_qualisys) - time_shift
        timestamp_wireshark_int = timestamp_wireshark[:, 1]
        timestamp_wireshark = timestamp_wireshark[:, 0]
...
#
        ...
        #    antennen_signale_Komplex[antennen_signale_Komplex < self.threshold_value] = 0
        ...
        doku_interpolation=[]
        if(np.isnan(time_xyz_antennen_Signal_Komplex).any()==True):
         ######cut NaN
            if(self.interpolation==True): # Values with NaN are not deleted! Certain consecutive counts of NaN are interpolated
                    tm_time_xyz_antennen_Signal_Komplex,doku_interpolation=load_than_save_qualisys_and_wireshark_data.NaN_interpolation(self=self,time_xyz_antennen_Signal_Komplex=time_xyz_antennen_Signal_Komplex,x_position_index=2)
            else:
            ...
            print("No NaN in the File! good File;)")
        ...
        print('The File have '+str(len(time_xyz_antennen_Signal_Komplex))+' Data Points')

        if(which_klass>0):
            if(self.cut_x_y_z_position_higher_then_antenna_null==False):
                time_xyz_antennen_Signal_Komplex_klass_cut,no_intervention = load_than_save_qualisys_and_wireshark_data.cut_x_y_z(self=self,
                    time_xyz_antennen_Signal=time_xyz_antennen_Signal_Komplex_klass)
            elif(self.cut_x_y_z_position_higher_then_antenna_null==True):
                time_xyz_antennen_Signal_Komplex_klass_cut, no_intervention = load_than_save_qualisys_and_wireshark_data.cut_x_y_z_position_higher_then_antenna_null(
                    self=self,
                    time_xyz_antennen_Signal=time_xyz_antennen_Signal_Komplex_klass)
            if(len(no_intervention)>0):
                no_intervention[:,41] = 0
        else:
            time_xyz_antennen_Signal_Komplex_klass_cut=[]
            no_intervention=time_xyz_antennen_Signal_Komplex_klass
            print("no Intervention")


        return time_xyz_antennen_Signal_Komplex_klass_cut, no_intervention,doku_interpolation
    ...
    def get_which_subject_intervened(File_list): #for Classifikation
        ...
        for i in range(0,len(File_list)):
            print('-1 = Different interventions (Test sequence)\n-2 = Outside past the shelf\n0 = No interventions')
        ...
        for i in range(0, len(time_xyz_antennen_Signal)):
            if(self.cut==True):
                if (self.x_cut[0] < int(round(time_xyz_antennen_Signal[i, 2])) < self.x_cut[1] and self.y_cut[0]  < int(
                    ...
                    no_intervention.append(time_xyz_antennen_Signal[i,:])
            else:
                tm_time_xyz_antennen_Signal.append(time_xyz_antennen_Signal[i, :])
        return np.array(tm_time_xyz_antennen_Signal),np.array(no_intervention)

    def cut_x_y_z_position_higher_then_antenna_null(self,time_xyz_antennen_Signal):
        tm_time_xyz_antennen_Signal = []
        no_intervention = []
        for i in range(0, len(time_xyz_antennen_Signal)):
            if(self.cut==True):
                if (self.x_cut[0] < int(round(time_xyz_antennen_Signal[i, 2])) < self.x_cut[1] and self.y_cut[0]  < int(
                    ...
                    time_xyz_antennen_Signal[i,9:41] = 0
                    tm_time_xyz_antennen_Signal.append(time_xyz_antennen_Signal[i, :])
            else:
                tm_time_xyz_antennen_Signal.append(time_xyz_antennen_Signal[i, :])
        return np.array(tm_time_xyz_antennen_Signal),np.array(no_intervention)

    def NaN_interpolation(self,time_xyz_antennen_Signal_Komplex,x_position_index):
        tm_time_xyz_antennen_Signal_Komplex = []
        doku_interpolation=[]
        first_value_after_NaN = 0
        for i in range(0, len(time_xyz_antennen_Signal_Komplex)):  # delet NAN rows and timestamp higher 0
            if (np.isnan(time_xyz_antennen_Signal_Komplex[i, x_position_index]) == False):
                tm_time_xyz_antennen_Signal_Komplex.append(time_xyz_antennen_Signal_Komplex[i, :])
            elif(np.isnan(time_xyz_antennen_Signal_Komplex[first_value_after_NaN, x_position_index]) == False):
                if(np.isnan(time_xyz_antennen_Signal_Komplex[i-1, x_position_index]) == False): #is_Value_bevor_NaN
                    begin_NaN = i
                    interp=0
                    while(self.interpolation_value>interp):
                        interp = interp + 1
                        if (np.isnan(time_xyz_antennen_Signal_Komplex[i + interp, x_position_index]) == False):
                            end_NaN = i + interp
                            print("interpolation was needed, counts:" + str(interp) +' (index from:'+str(i)+')')
                            doku_interpolation.append("interpolation was needed, counts:" + str(interp) +' (index from:'+str(i)+')')
                            break
                    #interpolieren
                    if(interp<self.interpolation_value):
                        begin_value = time_xyz_antennen_Signal_Komplex[begin_NaN-1,x_position_index:x_position_index+7]
                        end_value = time_xyz_antennen_Signal_Komplex[end_NaN,x_position_index:x_position_index+7]
                        diff_step = (end_value - begin_value)/(interp+1)
                        for p in range(0,interp):
                            time_xyz_antennen_Signal_Komplex[i+p,x_position_index:x_position_index+7] = begin_value +diff_step*(p+1)
                        tm_time_xyz_antennen_Signal_Komplex.append(time_xyz_antennen_Signal_Komplex[i, :])
                    else:
                        print("Interpolation Range is to small, NaN-Data-> delete")

                else:
                    print("No interpolation, index delete->" + str(i))
                    doku_interpolation.append("No interpolation, index delete->" + str(i))
            else:
                first_value_after_NaN = first_value_after_NaN+1
                print("First Values are NaN" + str(i))
                doku_interpolation.append("First Values are NaN" + str(i))
        return tm_time_xyz_antennen_Signal_Komplex,np.array(doku_interpolation)