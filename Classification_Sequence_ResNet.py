...
class Tensorflow_ResNet:
    ...
    def shape_the_Signal(self,X_Signal_number_train,X_train_time_series_normalize,Y_train_time_series_normalize,Class_train,i):
        ...
        for j in range(int(X_Signal_number_train[0]),int(X_Signal_number_train[-1] + 1)): #int(max(X_Signal_number_train))
            ...
            if(self.creat_dataset_time_steps*(2**i)<np.count_nonzero(X_Signal_number_train[:] == j)):
                X_train_time_series_tm, Y_train_time_series_tm,Class_train_tm = Tensorflow_ResNet.create_dataset(X_train_time_series_normalize[of:to][:],
        ...
        if(self.split_test_size*self.split_KFold_set<=1):
            if(i==1):
                if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                    ...
                    X_train, Y_train, X_Signal_number_train = Tensorflow_ResNet.cut_Signal_into_sequencen_train(self, X_train,Y_train,X_Signal_number_train)
                    X_test, Y_test, X_Signal_number_test = Tensorflow_ResNet.cut_Signal_into_sequencen_test(self, X_test,Y_test,X_Signal_number_test)
            ...
            X_train_time_series, Y_train_time_series = Tensorflow_ResNet.shape_the_Signal(self=self,
                                                                                   ...
                                                                                   i=(i - 1))
            X_test_time_series, Y_test_time_series = Tensorflow_ResNet.shape_the_Signal(self=self,
    ...
    def create_training_test_data(self,X_1,Y_1,X_Signal_number,Class,i):
        ...
        if(i==1):
            ...
            if(min_max_interval_split_data*2<len(X_1[0:count_Signal_end, :])):
                ...
                # KFold random
                X_train,X_test,Y_train,Y_test,X_Signal_number_train,X_Signal_number_test,Class_train,Class_test = Tensorflow_ResNet.create_training_test_data_random_KFold(self,X_1,Y_1,X_Signal_number,Class,count_Signal_beginn,count_Signal_end,min_max_interval_split_data)
                ...
                X_train, Y_train, X_Signal_number_train, Class_train = Tensorflow_ResNet.cut_Signal_into_sequencen_train_and_val(self,X_train,Y_train,X_Signal_number_train,Class_train)
                X_test, Y_test, X_Signal_number_test, Class_test = Tensorflow_ResNet.cut_Signal_into_sequencen_train_and_val(self, X_test, Y_test,X_Signal_number_test,Class_test)
        ...
        X_train_time_series, Y_train_time_series, Class_train = Tensorflow_ResNet.shape_the_Signal(self=self,
                                                                               X_Signal_number_train=X_Signal_number_train,
                                                                               X_train_time_series_normalize=X_train_time_series_normalize,
                                                                               Y_train_time_series_normalize=Y_train_time_series_normalize,
                                                                               Class_train=Class_train,
                                                                               i=(i - 1))
        X_test_time_series, Y_test_time_series, Class_test = Tensorflow_ResNet.shape_the_Signal(self=self,
                                                                             X_Signal_number_train=X_Signal_number_test,
                                                                             X_train_time_series_normalize=X_test_time_series_normalize,
                                                                             Y_train_time_series_normalize=Y_test_time_series_normalize,
    ...
    def resnet50_model(self,X_train_time_series, *args, **kwargs):
        ...
        output = Tensorflow_ResNet.conv_block(self,output, 3, [64, 64, 256], stage=2, block='a', strides=(1))
        output = Tensorflow_ResNet.identity_block(self,output, 3, [64, 64, 256], stage=2, block='b')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [64, 64, 256], stage=2, block='c')
        output = Tensorflow_ResNet.conv_block(self,output, 3, [128, 128, 512], stage=3, block='a')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [128, 128, 512], stage=3, block='b')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [128, 128, 512], stage=3, block='c')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [128, 128, 512], stage=3, block='d')
        output = Tensorflow_ResNet.conv_block(self,output, 3, [256, 256, 1024], stage=4, block='a')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [256, 256, 1024], stage=4, block='b')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [256, 256, 1024], stage=4, block='c')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [256, 256, 1024], stage=4, block='d')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [256, 256, 1024], stage=4, block='e')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [256, 256, 1024], stage=4, block='f')
        output = Tensorflow_ResNet.conv_block(self,output, 3, [512, 512, 2048], stage=5, block='a')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [512, 512, 2048], stage=5, block='b')
        output = Tensorflow_ResNet.identity_block(self,output, 3, [512, 512, 2048], stage=5, block='c')
        output = tf.keras.layers.GlobalAveragePooling1D(name='pool5')(output)
        output = tf.keras.layers.Dense(9,activation="softmax", name='fc1000')(output)
    ...
    def training_NN(self):
        ...
        for i in range(1, (self.time_steps_geo_folge + 1)):
            ...
            if(self.save==False):
                X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series, normalizer_x, normalizer_y, Class_train, Class_test = Tensorflow_ResNet.create_training_test_data(
            ...
            model = Tensorflow_ResNet.resnet50_model(self=self, X_train_time_series=X_train_time_series, input=32,
                                                     outputs=9)
            model.summary()
            time_stamp_from_NN =Tensorflow_ResNet.get_actuall_time_stamp()
            doku = []
            loss = 0
            batch = self.batch_size
            print_vali=[]
            print_train=[]
            for opti in range(0,3):
            #for loss in range(0,len(self.loss_funktions)):
                ...
                Tensorflow_ResNet.plot_2D_Classification(self, model, X_train_time_series, Y_train_time_series, Class_train, 0,
                                                  "Training", path_NN_tm_loss)

                Tensorflow_ResNet.plot_2D_Classification(self, model, X_test_time_series, Y_test_time_series, Class_test, 1,
                                                  "Test", path_NN_tm_loss)


                #doku_training =Tensorflow_ResNet.plot_3D_Classification(self, model, X_train_time_series, Y_train_time_series, Class_train, 0,
                #                                  "Training", path_NN_tm_loss)
                #doku_test = Tensorflow_ResNet.plot_3D_Classification(self, model, X_test_time_series, Y_test_time_series, Class_test, 1,
                #                                  "Test", path_NN_tm_loss)
...
if __name__ == '__main__':
    ...
    tensor = Tensorflow_ResNet(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
