...
class Gui_Signal_processing_programm(QAbstractItemModel):
    ...
    def merge_qualisys_and_wireshark_start(self):
        ...
        komplex_orentation_faktor = (np.exp(-1j * np.deg2rad(-27)) / 4.65)
        if(x_y_z_cut_range_error==False):
            lsmdqw = lqd.load_and_save_merge_data_qualisys_and_wireshark(merge_path,path_test,save_Name,komplex_orentation_faktor,cut_check_box,x_cut_range,y_cut_range,z_cut_range)
    ...
    def wireshark_window_load_data(self):
        ...
        if(os.path.isfile(file_name)==True):
            ...
            self.wireshark_window_ui.tableView_wireshark_window.setRowCount(len(self.wireshark_window_data[0:6000, 0]))
    ...
    def plot_wireshark_window(self):
        ...
        lookuptable_array = self.wireshark_window_data  # np.loadtxt(path, delimiter=';')
...
class look_up_table_model(QAbstractItemModel):

    Row_Batch_Count = 15

    def __init__(self,parent=None):
        super(look_up_table_model, self).__init__(parent=None)
