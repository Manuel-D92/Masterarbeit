...
class config_GoogLeNet_Tensorflow():
    # self.a = outside.
...
class GoogLeNet_Tensorflow:
    ...
    def cut_Signal_into_sequencen_test(self,X_test,Y_test,X_Signal_number_test):
        if (self.sequence_test == True):
            X_test_cut_tm = [];X_train_cut_NaN = [];
            Y_test_cut_tm = [];Y_train_cut_NaN = [];
            X_Signal_number_test_cut_tm = []; X_Signal_number_train_cut_NaN=[]
    ...
    def shape_the_Signal(self,X_Signal_number_train,X_train_time_series_normalize,Y_train_time_series_normalize,i):
        ...
        for j in range(int(X_Signal_number_train[0]),int(X_Signal_number_train[-1] + 1)):
            ...
            if(self.creat_dataset_time_steps*(2**i)<np.count_nonzero(X_Signal_number_train[:] == j)):
                ...
                if (j == int(X_Signal_number_train[first_data])):
                    if (len(X_train_time_series_tm) != 0):
                        X_train_time_series = X_train_time_series_tm
                        Y_train_time_series = Y_train_time_series_tm
                        print("lengh x_train_time_series" + str(len(X_train_time_series)))
                    else:
        ...
        if (self.split_test_size * self.split_KFold_set <= 1):
            if (i == 1):
                if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                    ...
                    X_train, Y_train, X_Signal_number_train = GoogLeNet_Tensorflow.cut_Signal_into_sequencen_train(self, X_train,Y_train,X_Signal_number_train)
                    X_test, Y_test, X_Signal_number_test = GoogLeNet_Tensorflow.cut_Signal_into_sequencen_test(self, X_test,Y_test,X_Signal_number_test)
            ...
            X_train_time_series, Y_train_time_series = GoogLeNet_Tensorflow.shape_the_Signal(self=self,
                                                                                   ...
                                                                                   i=(i - 1))
            X_test_time_series, Y_test_time_series = GoogLeNet_Tensorflow.shape_the_Signal(self=self,
            ...
            for s in range(1, last_Signal_number[0] + 1):
                if (np.count_nonzero(X_Signal_number == s) != 0):
                    ...
                    X_train_tm, X_test_tm, Y_train_tm, Y_test_tm = train_test_split(
                        ...
                        test_size=self.split_test_size, random_state=self.split_random_state,
                        shuffle=self.split_shuffle)
                    X_train = np.vstack((X_train, X_train_tm));
                    X_test = np.vstack((X_test, X_test_tm));
                    Y_train = np.vstack((Y_train, Y_train_tm));
    ...
    def create_training_data_sequence_end_complete_signal(self, X_1, Y_1, X_Signal_number, i):
        if (i == 1):
            X_train, X_test, Y_train, Y_test = train_test_split(X_1, Y_1, test_size=self.split_test_size,
                                                                random_state=self.split_random_state,
                                                                shuffle=self.split_shuffle)
            X_Signal_number_train, X_Signal_number_test = train_test_split(X_Signal_number,
                                                                           test_size=self.split_test_size,
                                                                           random_state=self.split_random_state,
                                                                           shuffle=self.split_shuffle)
            X_train, Y_train, X_Signal_number_train = GoogLeNet_Tensorflow.cut_Signal_into_sequencen_train(self,X_train,Y_train,X_Signal_number_train)
    ...
    def create_googLeNet(self,X_train_time_series,Y_train_time_series):
        ...
        x = tf.keras.layers.MaxPooling1D(3, padding='same', strides=2)(x)
        x = GoogLeNet_Tensorflow.inception_module(self=self, x=x,
                                                  filters_1x1=64,
        ...
        x = tf.keras.layers.Dense(Y_train_time_series.shape[1], name='output')(x)
    ...
    def training_NN(self):
        ...
        for i in range(1,(self.time_steps_geo_folge+1)):
            ...
            # Random KFold
            X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series, normalizer_x, normalizer_y = GoogLeNet_Tensorflow.create_training_data_random_KFold(
                self, X_1, Y_1, X_Signal_number, i)

            # KFold per steps
            # X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series,normalizer_x,normalizer_y = GoogLeNet_Tensorflow.create_training_data_KFold(self, X_1, Y_1, X_Signal_number, i)

            # old version (0.8-1) the last
            # X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series,normalizer_x,normalizer_y = GoogLeNet_Tensorflow.create_training_data_sequence_end_single_signal(self, X_1, Y_1, X_Signal_number, i)
            ...
            model = GoogLeNet_Tensorflow.create_googLeNet(self,X_train_time_series,Y_train_time_series)
            model.summary()
            ...
            for loss in range(0,len(self.loss_funktions)):
                ...
                try:
                    tf.keras.utils.plot_model(model, to_file=(path_NN_tm_loss + '\\model_GoogLeNet.png'), show_shapes=True, show_layer_names=True)
                except:
            ...
            tf.keras.utils.plot_model(model, to_file='current_model_GoogLeNet.png', show_shapes=True, show_layer_names=True)
...
if __name__ == '__main__':
    conf = config_GoogLeNet_Tensorflow
    googlenet_tensor = GoogLeNet_Tensorflow(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
