...
class config_RNN_Tensorflow_LSTM():
    # self.a = outside.
    ...
    create_dataset_overlap = 13  # example time_steps=16(0-15); Overlap from Feature to Feature example: 0= no overlap, 15= 0-15,1-16
...
class RNN_LSTM_Tensorflow:
    ...
    def cut_Signal_into_sequencen(self,X_train,Y_train,X_Signal_number_train,sequence_status):
        if (sequence_status == True):
            X_train_cut_tm = []
            Y_train_cut_tm = []
    ...
    def shape_the_Signal(self, X_Signal_number_train, X_train_time_series_normalize, Y_train_time_series_normalize, i):
        ...
        for j in range(int(X_Signal_number_train[0]),
            ...
            if (self.creat_dataset_time_steps * (2 ** i) < np.count_nonzero(X_Signal_number_train[:] == j)):
                X_train_time_series_tm, Y_train_time_series_tm = RNN_LSTM_Tensorflow.create_dataset(
        ...
        if (self.split_test_size * self.split_KFold_set <= 1):
            if (i == 1):
                if (min_max_interval_split_data * 2 < len(X_1[0:count_Signal_end, :])):
                    ...
                    X_train, Y_train, X_Signal_number_train = RNN_LSTM_Tensorflow.cut_Signal_into_sequencen(self,X_train,Y_train,X_Signal_number_train,self.sequence_training)
                    X_test, Y_test, X_Signal_number_test = RNN_LSTM_Tensorflow.cut_Signal_into_sequencen(self,X_test,Y_test,X_Signal_number_test,self.sequence_test)
            ...
            X_train_time_series, Y_train_time_series = RNN_LSTM_Tensorflow.shape_the_Signal(self=self,
                                                                                   ...
                                                                                   i=(i - 1))
            X_test_time_series, Y_test_time_series = RNN_LSTM_Tensorflow.shape_the_Signal(self=self,
    ...
    def training_NN(self):
        ...
        for i in range(1, (self.time_steps_geo_folge + 1)):
            ...
            # Random KFold
            X_train_time_series,Y_train_time_series,X_test_time_series,Y_test_time_series,normalizer_x,normalizer_y = RNN_LSTM_Tensorflow.create_training_data_random_KFold(self,X_1,Y_1,X_Signal_number,i)

            # KFold per steps
            # X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series,normalizer_x,normalizer_y = RNN_LSTM_Tensorflow.create_training_data_KFold(self, X_1, Y_1, X_Signal_number, i)

            # old version (0.8-1) the last
            #X_train_time_series, Y_train_time_series, X_test_time_series, Y_test_time_series, normalizer_x, normalizer_y = RNN_LSTM_Tensorflow.create_training_data_sequence_end_single_signal(self, X_1, Y_1, X_Signal_number, i)
            ...
            model = tf.keras.Sequential([#
                #Endcoder
                tf.keras.layers.LSTM(64,activation='relu',input_shape=(X_train_time_series.shape[1],X_train_time_series.shape[2])),#,return_sequences=True),
                #Decoder
            ...
            for loss in range(0, len(self.loss_funktions)):
                ...
                try:
                    tf.keras.utils.plot_model(model, to_file=path_NN_tm_loss + '\\model_RNN.png', show_shapes=True, show_layer_names=True)
                except:
...
if __name__ == '__main__':
    conf = config_RNN_Tensorflow_LSTM
    rnn_lstm = RNN_LSTM_Tensorflow(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
