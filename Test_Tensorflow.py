...
os.environ["PATH"] += os.pathsep + 'C:/Program Files/Graphviz 2.44.1/bin/' ### Path für model
...
class config_Test_Tensorflow():
    ...
    sequence_training_cut_x = [-20000, 170000]
    sequence_training_cut_y = [-20000, 140000]
    sequence_training_cut_z = [-40000, 40000]

    #### Testdata into Trainingssequence
    sequence_test = True
    sequence_test_cut_x = [-20000, 170000]
    sequence_test_cut_y = [-20000, 140000]
    sequence_test_cut_z = [-40000, 40000]
    ...
    #path = r"C:\Users\dauserml\Documents\2020_12_03_Training\03_alle_03_12_2020" #C:\Users\dauserml\Documents\Regression_all_01
    path_test_sequenz = r"C:\Users\dauserml\Documents\2020_11_16"
    #filename_training = "\\03_alle_03_12_2020.csv"#"\\all_Files_C_R.csv"  #Regression_all_01
    filename_test_sequenz = "\\2020_11_16_files_Z_800.csv"
    save_dir = '\\VGG_Time_Series_all_Data'
    ...
    training_Data_to = 51400  # Only "all_or_range_selection_Data = False"
    time = 1  # 1= Timesteps
    ...
    threshold_antenna = True
    threshold_value = 0.000045
...
class Tensorflow:
    ...
    @staticmethod
    def create_dataset(X, y, time_steps=1, overlap=1, offset=0):
    ...
    def cut_Signal_into_sequencen_train(self, X_train, Y_train, X_Signal_number_train):
        if (self.sequence_training == True):
            ...
            x_range_middle = abs(self.sequence_training_cut_x[0])+abs(self.sequence_training_cut_x[1])
            y_range_middle = abs(self.sequence_training_cut_y[0]) + abs(self.sequence_training_cut_y[1])
            z_range_middle = abs(self.sequence_training_cut_z[0]) + abs(self.sequence_training_cut_z[1])
            over_under_x = 0
            over_under_y = 0
            over_under_z = 0
            tm_Antenna_out_range=0#[]
            tm_xyz_out_range=0#[]
            ...
            for c in range(0, len(Y_train)):
                if (self.sequence_training_cut_x[0] < Y_train[c, 0] < self.sequence_training_cut_x[1]
                    ...
                    #tm_Antenna_out_range.append(X_train[c,:])
                    if (self.sequence_training_cut_x[0] > Y_train[c, 0]):
                        # under
                        over_under_x = self.sequence_training_cut_x[0] - 50

                    elif (Y_train[c, 0] > self.sequence_training_cut_x[1]):
                        # over
                        over_under_x = self.sequence_training_cut_x[1] + 50
                    else:
                        if (Y_train[c, 0] < x_range_middle):
                            over_under_x = self.sequence_training_cut_x[0] - 50
                        else:
                            over_under_x = self.sequence_training_cut_x[1] + 50
                    ...
                    if (self.sequence_training_cut_y[0] > Y_train[c, 1]):
                        # under
                        over_under_y = self.sequence_training_cut_y[0] - 50
                    elif (Y_train[c, 1] > self.sequence_training_cut_y[1]):
                        # over
                        over_under_y = self.sequence_training_cut_y[1] + 50
                    else:
                        if(Y_train[c, 1]<y_range_middle):
                            over_under_y = self.sequence_training_cut_y[0] - 50
                        else:
                            over_under_y = self.sequence_training_cut_y[1] + 50
                    ...
                    if (self.sequence_training_cut_z[0] > Y_train[c, 2]):
                        # under
                        over_under_z = self.sequence_training_cut_z[0] - 50
                    elif (Y_train[c, 2] > self.sequence_training_cut_z[1]):
                        # over
                        over_under_z = self.sequence_training_cut_z[1] + 50
                    else:
                        if (Y_train[c, 2] < z_range_middle):
                            over_under_z = self.sequence_training_cut_z[0] - 50
                        else:
                            over_under_z = self.sequence_training_cut_z[1] + 50
                    ...
                    #tm_xyz_out_range.append(Y_train[c, :])
                    #Y_train_cut_tm.append(Y_train[c, :])
                    #X_Signal_number_train_cut_tm.append(X_Signal_number_train[c, :])
...
###
            ...
            ##tm_Antenna_out_range = np.array(tm_Antenna_out_range)
            ##print("Training Max Antenna: "+str(np.max(tm_Antenna_out_range)))
            ##print("Training Min Atnenna: "+str(np.min(tm_Antenna_out_range)))
##
            ##tm_xyz_out_range = np.array(tm_xyz_out_range)
            ##print("x_y_z_out " +str(np.max(tm_xyz_out_range[:,0]))+" "+str(np.max(tm_xyz_out_range[:,1]))+" "+str(np.max(tm_xyz_out_range[:,2])))
            ##print("x_y_z_out " + str(np.min(tm_xyz_out_range[:, 0])) + " " + str(
            ##    np.min(tm_xyz_out_range[:, 1])) + " " + str(np.min(tm_xyz_out_range[:, 2])))
##
            ...
            #plt.subplot(4, 1, 2)
            #plt.plot(X_train[:, 2:4])
            #plt.subplot(4, 1, 3)
            #plt.plot(X_train[:, 28:30])
            #plt.subplot(4, 1, 4)
            #plt.plot(X_train[:, 30:32])
            #plt.figure(95)
            #plt.subplot(3, 1, 1)
            #plt.plot(Y_train[:, 0])
            #plt.subplot(3, 1, 2)
    ...
    def cut_Signal_into_sequencen_test(self, X_test, Y_test, X_Signal_number_test):
        if (self.sequence_test == True):
            ...
            x_range_middle = abs(self.sequence_test_cut_x[0]) + abs(self.sequence_test_cut_x[1])
            y_range_middle = abs(self.sequence_test_cut_y[0]) + abs(self.sequence_test_cut_y[1])
            z_range_middle = abs(self.sequence_test_cut_z[0]) + abs(self.sequence_test_cut_z[1])
            over_under_x = 0
            over_under_y = 0
            over_under_z = 0
            tm_Antenna_out_range=0#[]
            ...
            for c in range(0, len(Y_test)):
                if (self.sequence_test_cut_x[0] < Y_test[c, 0] < self.sequence_test_cut_x[1]
                    ...
                    #tm_Antenna_out_range.append(X_test[c, :])
                    if (self.sequence_test_cut_x[0] > Y_test[c, 0]):
                        # under
                        over_under_x = self.sequence_test_cut_x[0] - 50
                    elif (Y_test[c, 0] > self.sequence_test_cut_x[1]):
                        # over
                        over_under_x = self.sequence_test_cut_x[1] + 50
                    else:
                        if (Y_test[c, 0] < y_range_middle):
                            over_under_x = self.sequence_test_cut_x[0] - 50
                        else:
                            over_under_x = self.sequence_test_cut_x[1] + 50

                    if (self.sequence_test_cut_y[0] > Y_test[c, 1]):
                        # under
                        over_under_y = self.sequence_test_cut_y[0] - 50
                    elif (Y_test[c, 1] > self.sequence_test_cut_y[1]):
                        # over
                        over_under_y = self.sequence_test_cut_y[1] + 50
                    else:
                        if (Y_test[c, 1] < y_range_middle):
                            over_under_y = self.sequence_test_cut_y[0] - 50
                        else:
                            over_under_y = self.sequence_test_cut_y[1] + 50

                    if (self.sequence_test_cut_z[0] > Y_test[c, 2]):
                        # under
                        over_under_z = self.sequence_test_cut_z[0] - 50
                    elif (Y_test[c, 2] > self.sequence_test_cut_z[1]):
                        # over
                        over_under_z = self.sequence_test_cut_z[1] + 50
                    else:
                        if (Y_test[c, 2] < z_range_middle):
                            over_under_z = self.sequence_test_cut_z[0] - 50
                        else:
                            over_under_z = self.sequence_test_cut_z[1] + 50

                    #X_test_cut_tm.append(X_test[c, :])
                    #Y_test[c, :] = [over_under_x, over_under_y, over_under_z]
                    #Y_test_cut_tm.append(Y_test[c, :])
                    #X_Signal_number_test_cut_tm.append(X_Signal_number_test[c, :])
                    ...
                    #####flag = 0
                ####else:
                ####    if (self.sequence_training_cut_x[0] < Y_test[c, 0]):
                ####        # under
                ####        over_under_x = self.sequence_training_cut_x[0] - 10
                ####    elif (Y_test[c, 0] < self.sequence_training_cut_x[1]):
                ####        # over
                ####        over_under_x = self.sequence_training_cut_x[1] + 10
                ####    if (self.sequence_training_cut_y[0] < Y_test[c, 1]):
                ####        # under
                ####        over_under_y = self.sequence_training_cut_y[0] - 10
                ####    elif (Y_test[c, 1] < self.sequence_training_cut_y[1]):
                ####        # over
                ####        over_under_y = self.sequence_training_cut_y[1] + 10
                ####    if (self.sequence_training_cut_z[0] < Y_test[c, 2]):
                ####        # under
                ####        over_under_z = self.sequence_training_cut_z[0] - 10
                ####    elif (Y_test[c, 2] < self.sequence_training_cut_x[1]):
                ####        # over
                ####        over_under_z = self.sequence_training_cut_z[1] + 10
####
                ####    ## Not an NaN Training
            ...
            #tm_Antenna_out_range = np.array(tm_Antenna_out_range)
            #print("Test Max Antenna: " + str(np.max(tm_Antenna_out_range)))
            #print("Test Min Atnenna: " + str(np.min(tm_Antenna_out_range)))
    ...
    def create_training_data_random_KFold(self,X_1,Y_1,X_Signal_number,i):
        ...
        if(i==1):
            ...
            if(min_max_interval_split_data*2<len(X_1[0:count_Signal_end, :])):
                ...
                print("split interval from: " + str(count_Signal_beginn + int(min_max_interval_split_data)) + "   split interval to:" + str(
                    count_Signal_beginn + int(len(X_1[count_Signal_beginn:count_Signal_end, :]) - min_max_interval_split_data ))) #- int(len(X_1[count_Signal_beginn:count_Signal_end, :])*self.split_test_size)
                random_KFold_test_data_begin = int(random.uniform(int(min_max_interval_split_data),(int(len(X_1[0:count_Signal_end, :])-min_max_interval_split_data-int(len(X_1[0:count_Signal_end, :])*self.split_test_size)))))
                random_KFold_test_data_end = random_KFold_test_data_begin + int(self.split_test_size * len(X_1[0:count_Signal_end, :]))
                ...
                X_Signal_number_dynamisch = X_Signal_number
                X_Signal_number_neu = X_Signal_number_dynamisch+1
                X_Signal_number_train = np.vstack((X_Signal_number_dynamisch[training_data_1_idx],X_Signal_number_neu[training_data_2_idx]))
                X_Signal_number_dynamisch = X_Signal_number_dynamisch+1
                ...
                for s in range(1+X_Signal_number[0,0], last_Signal_number[0] + 1):
                    if(np.count_nonzero(X_Signal_number == s)!=0):
                        count_Signal_end = count_Signal_beginn + np.count_nonzero(X_Signal_number == s)
                        print("\nsplit interval from: "+str(count_Signal_beginn+int(min_max_interval_split_data)) +"   split interval to:" +str(count_Signal_beginn+int(
                            len(X_1[count_Signal_beginn:count_Signal_end, :]) - min_max_interval_split_data ))) #-int(len(X_1[count_Signal_beginn:count_Signal_end, :])*self.split_test_size)
                        random_KFold_test_data_begin = int(random.uniform((count_Signal_beginn+int(min_max_interval_split_data)), (count_Signal_beginn+int(
                            len(X_1[count_Signal_beginn:count_Signal_end, :]) - min_max_interval_split_data -int(len(X_1[count_Signal_beginn:count_Signal_end, :])*self.split_test_size)))))
                        ...
                        print("split from: " + str(random_KFold_test_data_begin) + "   split to: "+ str(random_KFold_test_data_end))
                        training_data_1_idx = np.arange(count_Signal_beginn, random_KFold_test_data_begin )
                        training_data_2_idx = np.arange(random_KFold_test_data_end , count_Signal_beginn+len(X_1[count_Signal_beginn:count_Signal_end, :]))
                        training_data_1_2_idx = np.hstack((training_data_1_idx, training_data_2_idx))
                        test_data_idx = np.arange(random_KFold_test_data_begin, random_KFold_test_data_end)
...
#
        ...
        if (self.threshold_antenna == True): ### Rauschen ausschneiden z.b. -0.0005 bis 0.0005 -auf 0 setzen
...
#
            ...
            #X_1 = medfilt2d(X_1,[17,1])

            X_1 = np.where(((-self.threshold_value) > X_1) | (X_1 > self.threshold_value), X_1, 0)
            X_1_tm, Y_1_tm, X_Signal_number_tm = [], [], []
            flag = 1
            for nul in range(0, len(X_1)):
                if (np.any(X_1[nul, :] != 0)):
                    X_1_tm.append(X_1[nul, :])
                    Y_1_tm.append(Y_1[nul, :])
                    X_Signal_number_tm.append(X_Signal_number[nul])
                    flag = 0
                else:
                    if (flag == 1):
                        X_Signal_number = X_Signal_number[:] + 1
                        flag = 0

            X_1 = np.array(X_1_tm)
            Y_1 = np.array(Y_1_tm)
            X_Signal_number = np.array(X_Signal_number_tm)
            plt.figure(93)
            plt.plot(X_1[:, 0:])
            plt.figure(94)
            plt.subplot(4,1,1)
        ...
        for i in range(1,(self.time_steps_geo_folge+1)):
            ...
            X_train_time_series,Y_train_time_series,X_test_time_series,Y_test_time_series,normalizer_x,normalizer_y = Tensorflow.create_training_data_random_KFold(self,X_1,Y_1,X_Signal_number,i)
            ...
            model = tf.keras.Sequential([#
                tf.keras.layers.Conv1D(64,3,strides=1, activation='relu',padding="same", input_shape=(X_train_time_series.shape[1],X_train_time_series.shape[2]),kernel_regularizer=regularizers.l2(0.001)),#,kernel_regularizer=regularizers.l2(0.001))
            ...
            for loss in range(0,len(self.loss_funktions)):
                ...
                try:
                    tf.keras.utils.plot_model(model, to_file=path_NN_tm_loss + '\\model_ResNet.png', show_shapes=True, show_layer_names=True)
                except:
                    print("Error save model.png")
                ...
                #Y_test_time_series_inverse = Y_test_time_series

                mse_test = np.mean((Y_pred_time_series[:, :] - Y_test_time_series_inverse[:, :])**2)
                print("MSE Test: " +  str(mse_test) + "mm")

                fig = plt.figure(1)
                plt.subplot(2,1,1)
                plt.title('X Position Test')
                plt.plot(Y_pred_time_series[:, 0])
                plt.plot(Y_test_time_series_inverse[:, 0])
                plt.legend(["X_pred","X_true"])
                plt.subplot(2,1,2)
                plt.plot(np.sqrt((Y_pred_time_series[:, 0]-Y_test_time_series_inverse[:, 0])**2))
                if(self.png==True):
                ...
                fig = plt.figure(2)
                plt.subplot(2, 1, 1)
                plt.title('Y Position Test')
                plt.plot(Y_pred_time_series[:, 1])
                plt.plot(Y_test_time_series_inverse[:, 1])
                plt.legend(["Y_pred", "Y_true"])
                plt.subplot(2, 1, 2)
                plt.plot(np.sqrt((Y_pred_time_series[:, 1] - Y_test_time_series_inverse[:, 1])**2))
                if (self.png == True):
                ...
                fig = plt.figure(3)
                plt.subplot(2, 1, 1)
                plt.title('Z Position Test')
                plt.plot(Y_pred_time_series[:, 2])
                plt.plot(Y_test_time_series_inverse[:, 2])
                plt.legend(["Z_pred", "Z_true"])
                plt.subplot(2, 1, 2)
                plt.plot(np.sqrt((Y_pred_time_series[:, 2] - Y_test_time_series_inverse[:, 2])**2))
                if (self.png == True):
                ...
                #Y_train_time_series_inverse = Y_train_time_series

                mse_train = np.mean((Y_pred_train_time_series[:, :] - Y_train_time_series_inverse[:, :]) ** 2)
                print("MSE Test: " + str(mse_train) + "mm")
                ...
                plt.plot(Y_train_time_series_inverse[:, 0])
                plt.legend(["X_pred", "X_true"])
                plt.subplot(2, 1, 2)
                plt.plot(np.sqrt((Y_pred_train_time_series[:, 0] - Y_train_time_series_inverse[:, 0])**2))
                if (self.png == True):
                    plt.savefig(path_NN_tm_loss + "_Train_X.png")
                if(self.pickle==True):
                    with open(path_NN_tm_loss+"_Train_X.pkl","wb") as fp:# Save Plots
                        pickle.dump(fig,fp,protocol=4)
                if(loss<len(self.loss_funktions)-1 or i<(self.time_steps_geo_folge)):
                    plt.close(fig=fig)
                fig = plt.figure(5)
                ...
                plt.plot(Y_train_time_series_inverse[:, 1])
                plt.legend(["Y_pred", "Y_true"])
                plt.subplot(2, 1, 2)
                plt.plot(np.sqrt((Y_pred_train_time_series[:, 1] - Y_train_time_series_inverse[:, 1])**2))
                if(self.png==True):
                    plt.savefig(path_NN_tm_loss + "_Train_Y.png")
                if(self.pickle==True):
                    with open(path_NN_tm_loss + "_Train_Y.pkl", "wb") as fp:  # Save Plots
                        pickle.dump(fig, fp, protocol=4)
                if(loss<len(self.loss_funktions)-1 or i<(self.time_steps_geo_folge)):
                    plt.close(fig=fig)
                fig = plt.figure(6)
                ...
                plt.plot(Y_train_time_series_inverse[:, 2])
                plt.legend(["Z_pred", "Z_true"])
                plt.subplot(2, 1, 2)
                plt.plot(np.sqrt((Y_pred_train_time_series[:, 2] - Y_train_time_series_inverse[:, 2])**2))
                if(self.png==True):
                    plt.savefig(path_NN_tm_loss + "_Train_Z.png")
                if(self.pickle==True):
                    with open(path_NN_tm_loss+"_Train_Z.pkl","wb") as fp:# Save Plots
                        pickle.dump(fig,fp,protocol=4)
                if(loss<len(self.loss_funktions)-1 or i<(self.time_steps_geo_folge)):
                    plt.close(fig=fig)
                ...
                doku.append("\nTime Steps: "+str(self.creat_dataset_time_steps*(2**(i-1)))+", Overlap: "+str(self.create_dataset_overlap)+", Offset: "+str(self.create_dataset_offset))
            ...
            tf.keras.utils.plot_model(model, to_file='current_model_VGG.png', show_shapes=True, show_layer_names=True)
