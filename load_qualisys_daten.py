...
class config_from_lqd_Script():  #load_qualisys_daten from Script
    path = r"C:\Users\User\Desktop\dauserml_Messungen_2020_07_22\Referenz_alfred"
    path_test = r"C:\Users\User\Desktop\dauserml_Messungen_2020_07_22\Referenz_alfred"
    name_file  = "sef"#"all_Files"
    ...
    z_cut = [-2000, 100]  # Z cut from, and to

class load_and_save_merge_data_qualisys_and_wireshark():
    def __init__(self,path,path_test,name_file,komplex_orentation_faktor,cut,x_cut,y_cut,z_cut):
    ...
    def covert_and_save_merge_data_qualisys_and_wireshark(self):
        ...
        time_xyz_antennen_Signal_Komplex_all_Files_list=[]
        if(len(folder_list)!=0):
            time_xyz_antennen_Signal_Komplex_all_Files =ltsqawd.load_than_save_qualisys_and_wireshark_data.load_than_save_qualisys_and_wireshark_data_to_learning(self=self,path=self.path,File=folder_list[0],which_klass=klass_list[0],number_Signal=0)
            for i in range(1,len(folder_list)):
                time_xyz_antennen_Signal_Komplex = (ltsqawd.load_than_save_qualisys_and_wireshark_data.load_than_save_qualisys_and_wireshark_data_to_learning(self=self,path=self.path,File=folder_list[i],which_klass=klass_list[i],number_Signal=i))
                if(len(time_xyz_antennen_Signal_Komplex)!=0):
                    ...
                    print("File: ",folder_list[i],"is empty (All Data are cut out!)")
            np.savetxt(('%s\\' % self.path + '%s.csv' % self.name_file), time_xyz_antennen_Signal_Komplex_all_Files, delimiter=';',fmt='%10.6E')
...
if __name__ == '__main__':
    ...
    lsmdqw = load_and_save_merge_data_qualisys_and_wireshark(config_from_lqd_Script.path,config_from_lqd_Script.path_test,
                                                              ...
                                                              config_from_lqd_Script.cut,config_from_lqd_Script.x_cut,config_from_lqd_Script.y_cut,config_from_lqd_Script.z_cut)
    #
