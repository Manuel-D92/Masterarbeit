...
model_classification = tf.keras.models.load_model(r"C:\Users\dauserml\Documents\2020_11_16_und_2020_12_03\VGG_Time_Series_Classification_Sequence_teil_1\NN_from_14_Dec_2020_08_04\sparse_categorical_crossentropy_Batch_4096/sparse_categorical_crossentropy.h5",compile = False)
...
normalizer_x = tensor.scaler  ### Wichtig! Aufpassen das der Richtige Scaler verwendet wird!!!
X_test_normalize = normalizer_x.fit_transform(X_test)
...
X_test_time_series, Y_test_time_series, Class_test,X_Signal_number_tr = CS.Tensorflow.shape_the_Signal(self=tensor,
...
# Regressions model prediction
if(x_y_z_to_Class==False):
    normalizer_x_Reg = Normalizer()  ### Wichtig! Aufpassen das der Richtige Scaler verwendet wird!!!

    ##conf = TT.config_Test_Tensorflow
    ##tensor_Reg = TT.Tensorflow(conf.creat_dataset_time_steps,conf.time_steps_geo_folge,conf.create_dataset_overlap,conf.create_dataset_offset,conf.sequence_training,
    ##                    conf.sequence_training_cut_x,conf.sequence_training_cut_y,conf.sequence_training_cut_z,conf.sequence_test,conf.sequence_test_cut_x,
    ##                    conf.sequence_test_cut_y,conf.sequence_test_cut_z,conf.path, conf.path_test_sequenz,conf.filename_training,
    ##                    conf.filename_test_sequenz, conf.save_dir, conf.all_or_range_selection_Data,
    ##                    conf.training_Data_from, conf.training_Data_to,
    ...
    ###X_test_reg, Y_test_reg,X_Signal_number_train_reg =TT.Tensorflow.cut_Signal_into_sequencen_train(tensor_Reg, X_1,Y_1,X_Signal_number)
##
    ##X_test_reg, Y_test_reg, X_Signal_number_train_reg,Class_reg = cut_Signal_into_sequencen_train(tensor_Reg, X_1,Y_1,X_Signal_number,Class)
    ##X_test, Y_test, X_Signal_number_train, Class_train = CS.Tensorflow.cut_Signal_into_sequencen_test_or_train(
    ##    self=tensor, X_test=X_test_reg, Y_test=Y_test_reg, Class_test=Class_reg, X_Signal_number_test=X_Signal_number_train_reg, test_or_train=True)
##
    X_test_normalize_Reg = normalizer_x_Reg.fit_transform(X_test)
    X_test_normalize_time_Reg = pd.DataFrame(X_test_normalize_Reg)
    Y_test_time_series_normalize_Reg = pd.DataFrame(Y_test)
    ...
    X_test_time_series_reg_class, Y_test_time_series_reg_class, Class_test_reg_class,X_Signal_number_trr = CS.Tensorflow.shape_the_Signal(self=tensor,
...
print("Y_test_time_series_reg_class"+str(len(Y_test_time_series_reg_class)))
...
CS.Tensorflow.plot_2D_Classification(tensor,model_classification,X_test_time_series,Y_test_time_series,Class_test,1,"Test",path)
CS.Tensorflow.plot_2D_Classification(tensor,model_Reg_classification,X_test_time_series_reg_class,Y_test_time_series_reg_class,Class_test_reg_class,2,"Test",path)


model_classification.compile(loss=tensor.loss_funktions[0],
              optimizer=tensor.optimizer[0],metrics=tensor.metrics)
...
Y_pred = np.argmax(model_classification.predict(X_test_time_series), axis=-1)
Y_pred_reg_class = np.argmax(model_Reg_classification.predict(X_test_time_series_reg_class), axis=-1)

Y_pred = pd.DataFrame(Y_pred,columns=["Y_Pred"])
Class_true = pd.DataFrame(Class_test,columns=["Class True"])
...
confusion_matrix = pd.crosstab(Y_pred["Y_Pred"],Class_true["Class True"])
...
plt.figure(5)
sns.heatmap(confusion_matrix,annot=True,vmax=120, fmt="d")
...
bal_erg = balanced_accuracy_score(Y_pred,Class_test)
print("balanced accuracy score: "+str(bal_erg))

erg = model_classification.evaluate(X_test_time_series,Class_test)
print("evaluate: "+str(erg))
