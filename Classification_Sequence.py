...
class config_Test_Tensorflow():
    ...
    filename_training = "\\Manuel_all_Jonas_Training.csv"#"\\all_Files_C_R.csv"


    path_test_sequenz = r"C:\Users\dauserml\Documents\2020_11_16"
    filename_test_sequenz = "\\2020_11_16_files_Z_800.csv"

    save_dir = "\\Manuel_all_Jonas_Training"#'\\VGG_Time_Series_Classification_Sequence_teil_1'
...
class Tensorflow:
    ...
    @staticmethod
    def create_dataset(X, y,Class,X_Signal_number=0, time_steps=1, overlap=1, offset=0):
        if (offset < time_steps and overlap < time_steps):
            ...
            for i in range(int((len(X) - (time_steps)) / overlap)+1):
                ...
                X_Sig.append(X_Signal_number[(i * overlap) + offset])
            return Xs, np.array(ys), np.array(Cl),np.array(X_Sig)
        else:
    ...
    def shape_the_Signal(self,X_Signal_number_train,X_train_time_series_normalize,Y_train_time_series_normalize,Class_train,i):
        ...
        for j in range(int(X_Signal_number_train[0]),int(X_Signal_number_train[-1] + 1)): #int(max(X_Signal_number_train))
            ...
            if(self.creat_dataset_time_steps*(2**i)<np.count_nonzero(X_Signal_number_train[:] == j)):
                X_train_time_series_tm, Y_train_time_series_tm,Class_train_tm,X_Signal_number_tm = Tensorflow.create_dataset(X_train_time_series_normalize[of:to][:],
                                                                                           Y_train_time_series_normalize[of:to][:],
                                                                                           Class_train[of:to],X_Signal_number=X_Signal_number_train[of:to],
                ...
                if (len(X_train_time_series_tm) != 0 and len(X_train_time_series) == 0):
                    ...
                    X_Signal_number = X_Signal_number_tm
                    ...
                    if(len(X_Signal_number_tm)!=0):
                        X_Signal_number = np.vstack((X_Signal_number, X_Signal_number_tm))
        return X_train_time_series, Y_train_time_series, Class,X_Signal_number
    ...
    def training_NN(self):
        ...
        for i in range(1, (self.time_steps_geo_folge + 1)):
            ...
            if(self.save==False):
                ...
                np.save(self.path+"\\Trainingsset_leave_one_out_x_Manuel_all_Jonas",Trainings_set_x)
                np.save(self.path + "\\Trainingsset_leave_one_out_y_Manuel_all_Jonas",Trainings_set_y)
                np.save(self.path + "\\Testset_leave_one_out_x_Manuel_all_Jonas", Test_set_x)
                np.save(self.path + "\\Testset_leave_one_out_y_Manuel_all_Jonas", Test_set_y)

            X_train_time_series = np.load(self.path+"\\Trainingsset_leave_one_out_x_Manuel_all_Jonas.npy")
            Training_set_y = np.load(self.path + "\\Trainingsset_leave_one_out_y_Manuel_all_Jonas.npy")
            X_test_time_series = np.load(self.path + "\\Testset_leave_one_out_x_Manuel_all_Jonas.npy")
            Test_set_y = np.load(self.path + "\\Testset_leave_one_out_y_Manuel_all_Jonas.npy")
